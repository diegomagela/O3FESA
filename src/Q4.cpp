                                           ...
                                           const double eta) const
{
    ...
    Eigen::VectorXd w_e = vct::std_to_eigen_vector(w_element);
...
Eigen::VectorXd Q4::thermal_force_resultants() const
{
    ...
    // std::vector<double> in_plane_thermal =
    //     get_section().get()->thermal_force_resultants_linear();
...
Eigen::VectorXd Q4::thermal_moment_resultants() const
{
    ...
    // Test linear variation
    // std::vector<double> moment_thermal =
    //     get_section().get()->thermal_moment_resultants_linear();
...
Eigen::MatrixXd Q4::linear_stiffness_matrix() const
{
    ...
    points = quadrature_shear.get_points();
    weights = quadrature_shear.get_weights();

    for (std::size_t i = 0; i < n_quadrature_points_shear; ++i)
    {
        for (std::size_t j = 0; j < n_quadrature_points_shear; ++j)
        {
            double xi_i = points.at(i);
            double eta_j = points.at(j);
            double w_i = weights.at(i);
...
Eigen::MatrixXd Q4::thermal_stiffness_matrix() const
{
    ...
    double NxT = thermal_forces(0);
    double NyT = thermal_forces(1);
    double NxyT = thermal_forces(2);

    // N_{delta T}
    Eigen::MatrixXd N_thermal = Eigen::MatrixXd::Zero(2, 2);
    N_thermal(0, 0) = NxT;
    N_thermal(0, 1) = NxyT;
    N_thermal(1, 0) = NxyT;
    N_thermal(1, 1) = NyT;
    ...
    for (std::size_t i = 0; i < n_quadrature_points; ++i)
    {
        for (std::size_t j = 0; j < n_quadrature_points; ++j)
        {
            ...
            K_dT += B_t.transpose() * N_thermal * B_t * d_omega;
        }
    }
...
Eigen::VectorXd Q4::internal_force_vector() const
{
    ...
    Eigen::VectorXd internal_force = -1.0 *
                                     (nonlinear_matrix +
                                      linear_matrix) *
                                     q_e;
